...
function write_help()
{
    ...
    echo "example:  sanity_check_manger.sh --iotagent sensors.geonovum.nl:8000 --contextbroker sensors.geonovum.nl:1026 -verbose 0"
    echo "other option: you can use environment variables  iotagent (export HOST_IOT=sensors.geonovum.nl:8000)"
    echo "                                                 contextbroker (export HOST_CB=sensors.geonovum.nl:1026)"
}
...
if test -z "$HOST_MAN" ; then
   export HOST_IOT=sensors.geonovum.nl:8000
fi

if test -z "$HOST_CB" ; then
   export HOST_CB=sensors.geonovum.nl:1026
fi
